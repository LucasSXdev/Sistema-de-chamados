...
export default function Dashboard() {
  ...
  async function handleMore() {
    setLoadingMore(true);
    const q = query(listRef, orderBy('created', 'desc'), startAfter(lastDocs), limit(2));
    const querySnapshot = await getDocs(q);
    await updateState(querySnapshot);
  } 
    ...
    <div>
      ...
      <div className="content">
        ...
        <>
                ...
                return (
                  <tr key={index}>
                    ...
                    <td data-label='Status'>
                      <span style={{ backgroundColor: item.status === 'aberto' ? '#5cb85c' : '#999' }}>{item.status}</span>
                    </td>
          ...
          {!loadingMore && !isEmpty && <button onClick={handleMore}>Mostrar mais chamados</button>}
        </>
        )}
      </div>
    </div>
